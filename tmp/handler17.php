...
class Shop_Payment_System_Handler17 extends Shop_Payment_System_Handler {
	...
	// Идентификатор сайта в системе Best2pay
	private $sector = 0;
	...
	public function getNotification() {
		...
		$best2pay_id = $this->sendRequest($this->getURL('/webapi/Register'), $data);
		...
		<div style="margin-top:40px;">
			<form accept-charset='utf8' action='<?php echo $this->getURL($payment_path)?>' method='post'>
				...
				<input type='hidden' name='signature' value='<?php echo base64_encode(md5($this->sector.$best2pay_id.$shop_cart_encoded.$this->password))?>'>
				<div style="display:flex;flex-direction:column;">
					<img style="display:block;margin:0 auto 20px;width:150px;height:50px;" src="https://best2pay.net/local/templates/best2pay_main_new/img/logo-blue.svg" alt="Best2pay" title="Платежная система Best2pay">
					<p>Оплата по банковской карте осуществляется через платежную систему <b><a target="_blank" href="https://best2pay.net">Best2pay</a></b>, обеспечивающую простой и безопасный способ оплаты по карте.
	...
	public function ProcessResult() {
		$best2pay_id = isset($_REQUEST['id']) ? $_REQUEST['id'] : null;
		$best2pay_operation = isset($_REQUEST['operation']) ? $_REQUEST['operation'] : null;
		...
		try {
			$signature = base64_encode(md5($sector_id . $best2pay_id . $best2pay_operation . $this->password));
			$data = [
				'sector' => $sector_id,
				'id' => $best2pay_id,
				'operation' => $best2pay_operation,
				'signature' => $signature
			];
